...
　チャレンジ＆レスポンス型認証サーバ（Diffie-Hellman鍵交換対応）
						                                          Ver. 4.0.0 '11 8/15

                      Copyright (C) 2005 Authd by Fumi.Iseki (TUIS). 
                                                 http://www.solar-system.tuis.ac.jp/
...
５．バグ，セキュリティホール，あるいは仕様と称するもの
　このサーバプログラムには以下の欠点があります．プログラムを使用する場合は，この欠点の意味する
ところを十分に理解して使用しすること．
...
６．ライセンス
　このプログラムは商用以外ではフリーです．再配布・ライブラリの再利用（改造を含む）は自由ですが，
再配布する場合は，配布されたものを完全な形で再配布すること．ライブラリの再利用では Copyright
を明記してください．
